...
export const updateExcursion = [
  ...
  async (req, res) => {
    const { id } = req.params;
    const { categoria, descripcion, destino, precio, duracion, transporte, comida, actividad } = req.body;

    try {
      const updateExcursion = await excursionSchema.updateOne({ _id: id }, 
        { $set: { categoria, descripcion, destino, precio, duracion, transporte, comida, actividad } });
