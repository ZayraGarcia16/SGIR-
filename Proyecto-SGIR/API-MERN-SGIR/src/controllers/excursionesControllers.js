...
export const updateExcursion = [
  ...
  async (req, res) => {
    ...
    try {
      const getExcursion = await excursionSchema.updateOne({ _id: id }, { $set: { categoria, descripcion, destino, precio, duracion, transporte, comida, actividad } });
      if (getExcursion.matchedCount === 0) {
        return res.status(404).json({ message: "Excursión no encontrada" });
      }
      if (getExcursion.modifiedCount === 0) {
