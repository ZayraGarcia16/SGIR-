...
function ComentariosCRUD() {
  ...
  useEffect(() => {
    const fetchComentarios = async () => {
      try {
        const response = await axios.get("http://localhost:9001/api/comentarios");
  ...
  const agregarComentario = async () => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:9001/api/comentarios",
        nuevoComentario
      );
  ...
  const eliminarComentario = async (id) => {
    try {
      await axios.delete(`http://localhost:9001/api/comentarios/${id}`);
  ...
  const guardarEdicion = async () => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:9001/api/comentarios/${_id}`,
        datosActualizar
      );
      setComentarios(
